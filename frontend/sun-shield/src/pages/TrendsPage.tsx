    ...
    const fetchUVData = async () => {
      ...
      try {
        const response = await fetch(
          `https://sunshield.azurewebsites.net/get_uv?location=${locationParam}`
        );
        ...
        if (response.ok) {
          ...
          if (typeof uvIndexValue === "number") {
            ...
            const generatedData = dates.map((date) => ({
              ...
              uvIndex: Math.max(
                0,
                Math.min(11, uvIndexValue + Math.floor(Math.random() * 3 - 1))
              ),
            }));
  ...
  return (
    <>
      ...
      >
        ...
        >
          Historical UV Trends:{" "}
          <span style={{ fontWeight: "normal" }}> {locationParam}</span>
        </h2>
